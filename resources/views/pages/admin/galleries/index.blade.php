            ...
            <div class="border-t border-gray-200 bg-white px-6 py-6 space-y-8">
                <section class="admin-module-section">
                    <div class="admin-module-section-header px-6 py-5">
                        <div>
                            <h3 class="admin-module-section-title">Album Galeri Desa</h3>
                            <p class="admin-module-section-description">Setiap album tampil sebagai card ringkas agar lebih mudah dipindai, diedit, dan dipublikasikan seperti modul berita.</p>
                        </div>
                    </div>
                    ...
                    <div class="px-6 pb-6">
                        <div class="grid gap-5 xl:grid-cols-2 2xl:grid-cols-3">
                            @forelse ($galleries as $gallery)
                                <article class="module-soft-card overflow-hidden rounded-[28px] border border-gray-200 bg-white shadow-md shadow-gray-200/60 transition duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-blue-100/50">
                                    <div class="relative aspect-[16/10] overflow-hidden bg-gray-100">
                                        <img src="{{ $gallery->cover_image_url ?: asset('img/bg.jpg') }}" alt="{{ $gallery->title }}" class="h-full w-full object-cover transition duration-700 hover:scale-105">
                                        <div class="absolute inset-x-0 top-0 flex items-start justify-between gap-3 p-4">
                                            <span class="inline-flex rounded-full bg-white/90 px-3 py-1 text-xs font-semibold text-blue-700 backdrop-blur ring-1 ring-white/40">
                                        ...
                                        @if ($gallery->is_featured)
                                            <span class="absolute bottom-4 left-4 inline-flex rounded-full bg-fuchsia-50 px-3 py-1 text-xs font-semibold text-fuchsia-700 ring-1 ring-fuchsia-100">
                                                Unggulan
                                            </span>
                                        @endif
                                    </div>
                                    ...
                                    <div class="p-5">
                                        <div class="flex flex-wrap items-center gap-3 text-xs font-semibold uppercase tracking-[0.18em]">
                                            <span class="text-blue-700">{{ $gallery->resolved_photo_count }} Foto</span>
                                            <span class="text-gray-400">{{ optional($gallery->gallery_date)->translatedFormat('d M Y') ?: 'Belum ada tanggal' }}</span>
                                        </div>

                                        <h3 class="mt-3 line-clamp-2 text-lg font-bold text-gray-900">
                                            {{ $gallery->title }}
                                        </h3>
                                        ...
                                        <p class="mt-3 line-clamp-3 text-sm leading-6 text-gray-600">
                                            {{ $gallery->excerpt ?: 'Belum ada ringkasan album.' }}
                                        </p>

                                        <div class="mt-4 grid gap-3 sm:grid-cols-3">
                                            <div class="rounded-2xl bg-gray-50 px-4 py-3">
                                                <p class="text-[11px] font-semibold uppercase tracking-[0.18em] text-gray-500">Lokasi</p>
                                                <p class="mt-2 line-clamp-2 text-sm font-medium text-gray-800">{{ $gallery->location_name ?: '-' }}</p>
                                            </div>
                                            <div class="rounded-2xl bg-gray-50 px-4 py-3">
                                                <p class="text-[11px] font-semibold uppercase tracking-[0.18em] text-gray-500">Urutan</p>
                                                <p class="mt-2 text-sm font-medium text-gray-800">{{ $gallery->sort_order }}</p>
                                            </div>
                                            <div class="rounded-2xl bg-gray-50 px-4 py-3">
                                                <p class="text-[11px] font-semibold uppercase tracking-[0.18em] text-gray-500">Publikasi</p>
                                                <p class="mt-2 text-sm font-medium text-gray-800">{{ $gallery->status === 'published' ? 'Tampil' : 'Tersimpan' }}</p>
                                            </div>
                                        </div>

                                        @if ($gallery->photos->isNotEmpty())
                                            <div class="mt-4 grid grid-cols-3 gap-2">
                                                @foreach ($gallery->photos->take(3) as $photo)
                                                    <div class="aspect-[4/3] overflow-hidden rounded-xl bg-gray-100">
                                                        <img src="{{ $photo->image_url }}" alt="{{ $photo->alt_text ?: $gallery->title }}" class="h-full w-full object-cover">
                                                    </div>
                                                @endforeach
                                            </div>
                                        @endif
                                        ...
                                        <div class="mt-5 flex flex-wrap gap-2 border-t border-gray-100 pt-4">
                                            @permission('galleries.edit')
                                                <button type="button" class="module-edit-btn" data-open-gallery-modal data-mode="edit" data-gallery-id="{{ $gallery->id }}">
                                                    Edit Album
                                                </button>
                                            @endpermission
                            ...
                            @empty
                                <div class="col-span-full rounded-[28px] border border-dashed border-gray-200 bg-gradient-to-br from-slate-50 to-white px-6 py-10 text-center text-sm text-gray-500">
                                    Belum ada album galeri. Tambahkan album pertama dari tombol di atas.
                                </div>
                            @endforelse
                        </div>
                    </div>
                ...
                <section class="admin-module-section">
                    <div class="admin-module-section-header px-6 py-5">
                        <div>
                            <h3 class="admin-module-section-title">Konten Publik Utama</h3>
                            <p class="admin-module-section-description">Album unggulan dan album terbaru yang paling mewakili tampilan galeri publik.</p>
                        </div>
                    </div>
                    <div class="grid gap-5 p-6 md:grid-cols-2 xl:grid-cols-3">
                        @foreach ($galleries->sortByDesc('is_featured')->take(6) as $gallery)
