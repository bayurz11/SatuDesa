...
@section('content')
    @php
        ...
        // Tags relasi (aman untuk eager/non-eager)
        $tags = collect();
        if ($post) {
        ...
        // Galeri dari meta.gallery (array URL/path)
        ...
        // Map embed: pakai accessor koordinat; fallback alamat
        $embedMap =
            $post?->map_embed_url ?:
            (filled($lokasi)
        ...
        $stat1 = data_get($post, 'meta.estimasi_produksi'); // ex: 180
        $stat1_unit = data_get($post, 'meta.estimasi_satuan', 'ton/tahun'); // ex: 'ton/tahun'
        $stat2 = data_get($post, 'meta.pelaku_umkm'); // ex: 42
        $stat3 = data_get($post, 'meta.luas_lahan'); // ex: 12.5

        // Deskripsi: utamakan body_html (trusted dari editor), fallback summary (escaped)
        $deskripsiHtml = filled($post?->body_html) ? $post->body_html : e($post?->summary);

        // Detail tabel kunci: eksplisit + meta.detail
        $fmtRp = function ($n) {
            return is_numeric($n) ? 'Rp ' . number_format($n, 0, ',', '.') : $n;
        };
            ...
            <div class="grid md:grid-cols-12 gap-0">
                ...
                <div class="md:col-span-5 p-6 md:p-8 flex flex-col justify-center">
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        @foreach ($tags as $t)
                            <span
                                class="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-medium text-emerald-700 ring-1 ring-emerald-200">#{{ $t }}</span>
                        @endforeach
                    </div>

                    <h1 class="mt-3 text-2xl md:text-3xl font-extrabold tracking-tight text-gray-900">{{ $post->title }}
                    </h1>
                    ...
                    <div class="mt-5 flex flex-col sm:flex-row gap-2">
                        ...
                        {{-- Mobile: hijau solid; mulai md: outline --}}
                                            ...
                                            <td class="px-4 py-3 text-gray-800">
                                                @if ($k === 'Tautan Eksternal' && filled($v))
                                                    <a href="{{ $v }}" target="_blank" rel="noopener"
                                                        class="text-green-700 hover:text-green-800 underline">{{ $v }}</a>
                                                @else
