    ...
    <div class="bg-gradient-to-r from-green-50 via-green-100 to-purple-50 px-6 py-6 border-b border-gray-200">
        <div class="flex justify-between items-center">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h2 class="text-2xl font-bold text-gray-900">Sejarah Management</h2>
                    <p class="text-sm text-gray-600 mt-1">Kelola informasi sejarah desa</p>
                </div>
            </div>

            @permission('profil.create')
                <button x-data @click="window.Livewire.dispatch('openSejarahForm', { id: null })"
                ...
                @forelse($sejarah as $sejarahItem)
                    <tr
                        ...
                        <td class="px-6 py-5 whitespace-nowrap">
                            <div class="flex items-center">
                                ...
                                <div
                                    ...
                                    @if ($sejarahItem->gambar)
                                        <img src="{{ asset('storage/' . $sejarahItem->gambar) }}" alt="Foto Sejarah"
                                ...
                                <div>
                                    <div class="text-sm font-semibold text-gray-900">
                                        {{ Str::words(html_entity_decode(strip_tags($sejarahItem->isi)), 6, '...') }}
                                    </div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-5 whitespace-nowrap">
                            ...
                            {{ $sejarahItem->is_active ? 'bg-gradient-to-r from-green-100 to-emerald-100 text-green-800' : 'bg-gradient-to-r from-red-100 to-pink-100 text-red-800' }}">
                                <div
                                    class="w-2 h-2 rounded-full mr-2 {{ $sejarahItem->is_active ? 'bg-green-500 animate-pulse' : 'bg-red-500' }}">
                                </div>
                                {{ $sejarahItem->is_active ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        ...
                        <td class="px-6 py-5 whitespace-nowrap">
                            <div class="flex items-center text-sm text-gray-500">
                                ...
                                <div>
                                    <div class="font-medium">{{ $sejarahItem->created_at->format('M d, Y') }}</div>
                                    <div class="text-xs">{{ $sejarahItem->created_at->diffForHumans() }}</div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-5 whitespace-nowrap text-sm font-medium">
                            <div class="flex space-x-2">
                                {{-- VIEW --}}
                                <button wire:click="$dispatch('openSejarahModal')"
                                ...
                                @permission('profil.edit')
                                    <button x-data
                                        @click="window.Livewire.dispatch('openSejarahForm', { id: {{ $sejarahItem->id }} })"
                                ...
                                @permission('profil.edit')
                                    <button type="button"
                                        x-on:click="$wire.call('toggleSejarahStatus', {{ $sejarahItem->id }})"
                                        ...
                                        {{ $sejarahItem->is_active ? 'Deactivate' : 'Activate' }}
                                    </button>
                                @endpermission

                                {{-- DELETE --}}
                                @permission('profil.delete')
                                    <button wire:click="performDeleteSejarah({{ $sejarahItem->id }})"
                                        wire:loading.attr="disabled"
                                        class="inline-flex items-center px-3 py-2 text-xs font-semibold text-red-600 bg-red-50 rounded-lg hover:bg-red-100 hover:text-red-700 transition-all duration-200 transform hover:scale-105 disabled:opacity-50"
                                        title="Delete">
                                        <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-16 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <h3 class="text-xl font-semibold text-gray-900 mb-2">No sejarah found</h3>
                                <p class="text-gray-500 mb-6 max-w-sm text-center">
                                    @if ($search || !$showInactive)
                                        Try adjusting your search or filter criteria to find what you're looking for.
                                    @else
                                        Get started by creating your first sejarah entry.
                                    @endif
                                </p>
    ...
    <div class="px-6 py-4 bg-gradient-to-r from-gray-50 to-green-50 border-t border-gray-200 rounded-b-2xl">
        <div class="flex items-center justify-between">
            <div class="text-sm text-gray-600">
                ...
                to <span class="font-medium">{{ $sejarah->lastItem() ?? 0 }}</span>
                of <span class="font-medium">{{ $sejarah->total() }}</span> sejarah
            </div>
