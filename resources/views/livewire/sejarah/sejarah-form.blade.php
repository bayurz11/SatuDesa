              ...
              <div class="relative top-8 mx-auto w-full max-w-4xl p-6 border shadow-lg rounded-md bg-white">
                  <div class="max-h-[80vh] overflow-y-auto">
                      <div class="mt-3">
                          ...
                          <form id="sejarah-form-{{ $wrapId }}" wire:submit.prevent="save" class="space-y-6">
                              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                  ...
                                  <div class="col-span-2 flex justify-end gap-3 pt-6 border-t border-gray-200">
                                      <button type="button" wire:click="closeModal"
                                          class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-200 rounded-md hover:bg-gray-300">Cancel</button>
                                      <button type="submit"
  ...
  @push('scripts')
      <script>
          // Saat Trix siap, pastikan konten awal termuat dari hidden (penting untuk edit)
          document.addEventListener('trix-initialize', (e) => {
              const inputId = e.target.getAttribute('input');
              const hidden = document.getElementById(inputId);
              if (hidden) {
                  // Muat HTML awal ke editor (kalau Livewire sudah inject nilai isi)
                  try {
                      e.target.editor.loadHTML(hidden.value || '');
                  } catch {}
                  // Tambah guard: sebelum submit, paksa dispatch input agar Livewire mengirim "isi"
          ...
          // Setiap ada perubahan di Trix → dispatch 'input' ke hidden input yang terkait
