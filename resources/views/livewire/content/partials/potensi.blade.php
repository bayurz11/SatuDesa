@section('content')
    <section class="mx-auto max-w-7xl px-4 md:px-6 lg:px-8 py-10 md:py-14" data-aos="fade-up"> {{-- Breadcrumb --}} <nav
            class="mb-6 mt-8 md:mb-8 text-sm text-gray-500" aria-label="Breadcrumb">
        ...
        </nav> {{-- Heading --}} <header class="text-center mb-10 md:mb-12">
            <h1 class="text-3xl md:text-4xl font-extrabold tracking-tight text-green-700"> Potensi Desa Mentuda </h1>
            <p class="mt-3 md:mt-4 text-gray-600 md:text-lg max-w-2xl mx-auto"> Gambaran potensi ekonomi, pariwisata,
                pertanian, perikanan, industri kreatif, hingga lingkungan yang menjadi kekuatan Desa Mentuda. </p>
        </header>
        <div class="grid gap-8 lg:grid-cols-3 items-start"> {{-- KONTEN UTAMA --}} <article class="lg:col-span-2 space-y-8">
                {{-- Highlight / Hero Potensi --}} <div class="relative overflow-hidden rounded-2xl bg-white shadow ring-1 ring-black/5">
                    <div class="grid md:grid-cols-12 gap-0">
                        <figure class="md:col-span-7 h-56 md:h-72 overflow-hidden"> <img src="public/img/potensi2.jpg"
                                alt="Panorama Pariwisata" class="h-full w-full object-cover" loading="lazy"
                                decoding="async"> </figure>
                        <div class="md:col-span-5 p-6 md:p-8 flex flex-col justify-center"> <span
                                ...
                                <x-heroicon-o-sparkles class="size-4" /> Sorotan </span>
                ...
                </div> {{-- Filter kategori (placeholder) --}} <div class="bg-white rounded-xl shadow p-4 ring-1 ring-black/5">
                ...
                @endphp <div class="grid gap-6 sm:grid-cols-2">
                        ...
                        <div x-data="{ open: false }"
                            ...
                            <div class="p-4">
                                <div class="mb-1 flex flex-wrap items-center gap-2"> <span
                                        ...
                                        <x-heroicon-o-tag class="size-4" /> {{ $p['tag'] }} </span> </div>
                                ...
                                <div class="mt-3 flex items-center justify-between"> <a href="#"
                                        ...
                                        <x-heroicon-o-eye class="size-4" /> Detail </a> <button @click="open=true"
                                        ...
                                        <x-heroicon-o-arrows-pointing-out class="size-4" /> Pratayang </button> </div>
                            </div> {{-- Modal Preview --}} <div x-show="open" x-transition x-cloak @click.self="open=false"
                                ...
                                <div class="relative max-w-xl w-full bg-white rounded-2xl shadow ring-1 ring-black/5 p-6">
                                    ...
                                    <div class="mt-4 flex items-center gap-2"> <a href="#"
                                            ...
                                            <x-heroicon-o-link class="size-4" /> Buka Halaman </a> <a href="#"
                                            ...
                                            <x-heroicon-o-share class="size-4" /> Bagikan </a> </div>
                                </div>
                            </div>
                        </div>
                    @endforeach
                </div> {{-- CTA --}} <div class="rounded-2xl border border-green-200 bg-green-50/60 p-5 md:p-6">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                        ...
                        <div class="flex items-center gap-2"> <a href="#"
                                class="inline-flex items-center gap-2 rounded-lg bg-green-600 px-4 py-2 text-sm font-medium text-white hover:bg-green-700 transition">
                                <x-heroicon-o-paper-airplane class="size-4" /> Kirim Data </a> <a href="#"
                                class="inline-flex items-center gap-2 rounded-lg border border-green-600 px-4 py-2 text-sm font-medium text-green-700 hover:bg-green-600 hover:text-white transition">
                                <x-heroicon-o-inbox class="size-4" /> Kontak Admin </a> </div>
                    </div>
                </div>
            </article> {{-- SIDEBAR --}} <aside class="space-y-6 lg:sticky lg:top-20">
                <div class="bg-white rounded-xl shadow p-4 ring-1 ring-black/5">
                    ...
                    <ul class="space-y-2 text-sm text-gray-700">
                        <li class="flex items-center gap-2"> {{-- chart-bar --}} <svg xmlns="http://www.w3.org/2000/svg"
                                class="h-4 w-4 text-green-700" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                    d="M3 19.5h18M6 17V9m6 8V5m6 12v-6" />
                            </svg> Ekonomi &amp; UMKM </li>
                        <li class="flex items-center gap-2"> {{-- map --}} <svg
                                xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-green-700" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                    d="M9 20.25l-6-2.25V5.25l6 2.25m0 12.75l6-2.25m-6 2.25V7.5m6 10.5l6 2.25V8.25l-6-2.25m0 12.75V6" />
                            </svg> Pariwisata Alam </li>
                        <li class="flex items-center gap-2"> {{-- globe-alt --}} <svg
                                xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-green-700" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                    d="M12 21a9 9 0 100-18 9 9 0 000 18zm0 0c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3 7.5 7.03 7.5 12s2.015 9 4.5 9zm-7.5-9h15" />
                            </svg> Pertanian &amp; Lingkungan </li>
                        <li class="flex items-center gap-2"> {{-- camera (sebagai ikon “kreatif”) --}} <svg
                                xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-green-700" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
