    ...
    <div class="bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50 px-6 py-6 border-b border-gray-200">
        <div class="flex justify-between items-center">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h2 class="text-2xl font-bold text-gray-900">Manajemen Potensi Desa</h2>
                    <p class="mt-1 text-sm text-gray-600">Kelola data potensi unggulan, status publikasi, dan kategori potensi desa</p>
                </div>
            </div>
            ...
            <div class="flex items-center space-x-3">
                ...
                <button wire:click="$dispatch('openPotentialForm')"
                        ...
                        <div class="flex-1">
                            ...
                            <div class="mt-4 flex items-center justify-between">
                                <div class="flex items-center space-x-2">
                                    ...
                                    <span class="text-sm text-green-600 font-semibold">Siap dilihat publik</span>
                                </div>
                        ...
                        <div class="flex-1">
                            <div class="flex items-center space-x-4">
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="text-xl font-bold text-gray-900">{{ $stats['review'] ?? 0 }}</p>
                                </div>
                            </div>
                            ...
                            <div class="mt-4 flex items-center justify-between">
                                <div class="flex items-center space-x-2">
                                    ...
                                    <span class="text-sm text-purple-600 font-semibold">Menunggu penyuntingan</span>
                                </div>
            ...
            <div class="flex-1">
                <div class="relative">
                    ...
                    <input wire:model.live="search" type="text"
                        placeholder="Cari judul, lokasi, ringkasan, atau isi potensi..."
    ...
    <div class="bg-white p-6">
            ...
            <div
                ...
                <div class="relative h-40 shrink-0 overflow-hidden bg-gray-100">
                    @if ($potential->cover_image_url)
                        <img src="{{ $potential->cover_image_url }}" alt="{{ $potential->cover_image_alt ?: $potential->title }}"
                    ...
                    @else
                        <div
                            ...
                            <span class="text-2xl font-bold text-white">
                                {{ strtoupper(substr($potential->title, 0, 2)) }}
                            </span>
                        </div>
                    @endif
                    ...
                    <div class="absolute top-4 left-4 flex flex-wrap gap-2">
                        <span
                            ...
                            {{ $potential->category?->name ?? 'Tanpa kategori' }}
                        </span>
                        ...
                        <span
                            ...
                            {{ $potential->status === 'published' ? 'Published' : ($potential->status === 'review' ? 'Review' : 'Draft') }}
                        </span>
                    </div>
                    ...
                    <div class="absolute bottom-0 left-0 right-0 p-4">
                        ...
                        <p class="mt-1 text-[11px] text-white/80">
                            {{ $potential->location_name ?: ($potential->village?->name ?? 'Lokasi belum diisi') }}
                            <span class="mx-1">&bull;</span>{{ $potential->updated_at->diffForHumans() }}
                        </p>
                    </div>
                </div>
                ...
                <div class="flex flex-1 flex-col p-5">
                    ...
                    <div class="mt-4 min-h-[3.5rem] space-y-1 text-sm text-gray-500">
                        <p><span class="font-medium text-gray-700">Jenis:</span> {{ $potential->potential_type ?: '-' }}</p>
                        <p><span class="font-medium text-gray-700">Kontak:</span> {{ $potential->contact_person ?: '-' }}</p>
                    </div>
                    ...
                    <div class="mt-4 flex items-center border-t border-gray-100 pt-4">
                        <div
                            ...
                            <span class="text-sm font-bold text-white">
                                {{ strtoupper(substr($potential->village?->name ?? 'D', 0, 1)) }}
                            </span>
                        </div>

                        <div>
                            <div class="text-sm font-medium text-gray-900">
                                {{ $potential->village?->name ?? '-' }}
                            </div>

                            <div class="text-xs text-gray-500">
                                {{ $potential->development_status ?: '-' }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-4 border-t border-gray-100 pt-4">
                        <div class="flex flex-wrap gap-2">
                            <button wire:click="$dispatch('openPotentialForm', { potentialId: {{ $potential->id }} })"
                            ...
                            @if ($potential->status !== 'published')
                                <button wire:click="publishPotential({{ $potential->id }})"
                            ...
                            @else
                                <button wire:click="moveToDraft({{ $potential->id }})"
                            ...
                            <button wire:click="confirmDeletePotential({{ $potential->id }})"
        ...
        @empty
            <div class="flex flex-col items-center justify-center py-16">
                ...
                <h3 class="text-xl font-semibold text-gray-900 mb-2">Belum ada data potensi</h3>

                <p class="text-gray-500 mb-6 max-w-sm text-center">
                    @if ($search || $status || $categoryId)
                        Coba ubah kata kunci atau filter untuk menemukan potensi yang dicari.
                    @else
                        Mulai dengan membuat data potensi desa pertama untuk website publik.
                    @endif
                </p>
    ...
    <div class="px-6 py-4 bg-gradient-to-r from-gray-50 to-blue-50 border-t border-gray-200 rounded-b-2xl">
        <div class="flex items-center justify-between gap-4">
            <div class="text-sm text-gray-600">
                Menampilkan <span class="font-medium">{{ $potentials->firstItem() ?? 0 }}</span> sampai <span
                    class="font-medium">{{ $potentials->lastItem() ?? 0 }}</span> dari <span
                    class="font-medium">{{ $potentials->total() }}</span> data potensi
            </div>
