            ...
            <div class="flex items-center space-x-4">
                <div
                    ...
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8.5 8.5L4 8" />
                    </svg>
                </div>
                <div>
                    <h2 class="text-2xl font-bold text-gray-900">Tags</h2>
                    <p class="text-sm text-gray-600 mt-1">Kelola tag untuk Berita, Pengumuman & Potensi</p>
                </div>
            </div>

            @permission('informasi.create')
                <button wire:click="$dispatch('openTagForm')"
            ...
            <tbody class="bg-white divide-y divide-gray-100">
                @forelse($data as $item)
                    <tr
                        ...
                        <td class="px-6 py-5 whitespace-nowrap font-semibold text-gray-900">{{ $item->name }}</td>
                        <td class="px-6 py-5 whitespace-nowrap text-gray-800">{{ $item->slug }}</td>
                        ...
                        <td class="px-6 py-5 whitespace-nowrap text-sm font-medium">
                            <div class="flex space-x-2">
                                @permission('informasi.edit')
                                    <button wire:click="$dispatch('openTagForm', { id: @js($item->id) })"
                                ...
                                <div
                                    ...
                                    <svg class="w-10 h-10 text-gray-400" fill="none" stroke="currentColor"
                                        ...
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                            d="M20 7l-8.5 8.5L4 8" />
                                    </svg>
                                </div>
                                <h3 class="text-xl font-semibold text-gray-900 mb-2">Belum ada tag</h3>
                                <p class="text-gray-500 mb-6 max-w-sm text-center">
                                    @if ($search || $showInactive)
                                        Coba ubah kata kunci atau filter.
                                    @else
                                        Tambahkan tag pertama untuk klasifikasi konten.
                                    @endif
                                </p>
